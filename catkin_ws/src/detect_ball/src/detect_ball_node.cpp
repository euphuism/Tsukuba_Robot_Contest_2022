...
public:
  ...
  {
    ros::NodeHandle nh;
    pub_cdn_ = nh.advertise<geometry_msgs::Twist>("cmd_vel", 5);　//chcange message type
		sub_scan_ = nh.subscribe("scan", 5, &RSJRobotTestNode::cbScan, this);
  }
  void mainloop()
  {
	    ...
	    // T = F^(-1) * G
			int T[3] = 
				{((sumy2 * i     - sumy  * sumy) * sumx3_xy2 + (sumxy * i    - sumx  * sumy ) * sumx2y_y3 + (sumx  * sumy  - sumxy * sumy ) * sumx2_y2) / detF, 
				 ...
				 ((sumx  * sumy2 - sumxy * xumy) * sumx3_xy2 + (sumx2 * sumy - sumx  * sumxy) * sumx2y_y3 + (sumxy * sumxy - sumx2 * sumy2) * sumx2_y2) / detF};
			
			cx = double(T[0] / (-2));
			cy = double(T[1] / (-2));
			r  = std::sqrt(cx * cx + cy * cy - T[2]);
			
			hoge_msgs::BallCDN ball_cdn;
			ball_cdn.cx_ = cx;
			ball_cdn.cy_ = cy;
			ball_cdn.r_  = r;
			
			pub_cdn.publish(ball_cdn);
